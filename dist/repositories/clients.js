...
exports.default = {
    async findByDocument(document) {
        const result = await database_1.default.query("SELECT id, document FROM clients WHERE document = $1", [document]);
        return result.rows[0] || null;
    },
    async createClient(document) {
        const result = await database_1.default.query("INSERT INTO clients (document) VALUES ($1) RETURNING id, document", [
            document,
        ]);
